...
class DeepGBoostRegressor(BaseEstimator, ClassifierMixin):

    """
    Distributed Gradient Booted Forest algorithm.
    """
    ...
    def _predict_stage(self, X):
        ...
        pred = np.full((X.shape[0],self.n_trees), self.prior_)
        for l, layer in enumerate(self.graph_):
            for t, tree in enumerate(layer):
                tree_pred_o = tree.predict(X).reshape((X.shape[0],-1))
                tree_pred = (tree_pred_o*self.weights_[l][t]).sum(axis=1)
                pred[:,t] += tree_pred
    ...
    def fit(self,X,y):
        ...
        for l in range(self.n_layers):
            ...
            # New layer
            self.graph_.append(self._fit_stage(X, pseudo_y_o, l))
